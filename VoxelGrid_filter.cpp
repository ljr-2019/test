 ...
 main (int argc, char** argv)
{
  ...
  reader.read( "tunnel1.pcd", *cloud2_ptr);
  if(cloud2_ptr==NULL) { std::cout << "pcd file read err" << endl; return -1;}
  std::cout << "PointCLoud before filtering 滤波前数量: " << cloud2_ptr->width * cloud2_ptr->height
  ...
  // 输出滤波后的点云信息
  std::cout << "PointCLoud before filtering 滤波后数量: " << cloud2_filtered_ptr->width * cloud2_filtered_ptr->height
       << " data points ( " << pcl::getFieldsList (*cloud2_filtered_ptr) << "." << endl;
  // 写入内存
  pcl::PCDWriter writer;
  writer.write("filtered_tunnel.pcd",*cloud2_filtered_ptr,
